...
**Antes de empezar, lee:** [07 — Adaptadores](../02-conceptos/07-adaptadores.md), [14 — RecyclerView](../02-conceptos/14-recyclerview.md) y los proyectos [EjercicioAdaptadoresFinal](../05-proyectos-alumno/EjercicioAdaptadoresFinal.md) y [AdapterDam2](../05-proyectos-alumno/AdapterDam2.md), de donde salen estos ejercicios.
...
![equipos](../adjuntos/img/n3-1-equipos.png)

### 🎯 Qué practicas
Clase modelo `Serializable`, `BaseAdapter` con sus 4 métodos, `LayoutInflater.inflate(…, parent, false)` ([07 §2](../02-conceptos/07-adaptadores.md)).
...
`getView` se llama **una vez por cada fila visible** (y otra vez al hacer scroll). Tus adaptadores no usan `convertView` (siempre inflan una vista nueva), como `EquiposAdapter` de clase; para listas cortas no se nota ([07 §3](../02-conceptos/07-adaptadores.md)).
...
![jugadores](../adjuntos/img/n3-2-jugadores.png)

### 🎯 Qué practicas
`GridView` (`android:numColumns`), otro `BaseAdapter`, recibir un objeto `Serializable` con `getSerializableExtra` y casting ([01 §2.3](../02-conceptos/01-fundamentos-bundle-intent-ciclo-vida.md)).
...
**Explicación:**
- El **casting** `(Equipo)` es necesario porque `getSerializableExtra` devuelve un tipo general ([00 §12](../02-conceptos/00-programacion-basica.md)).
...
![detalle](../adjuntos/img/n3-3-detalle-jugador.png)

### 🎯 Qué practicas
`Bundle.putSerializable`, `intent.putExtras`, dos niveles de navegación, `ConstraintLayout` en cascada ([03 §3](../02-conceptos/03-constraintlayout.md)).
...
![spinner](../adjuntos/img/n3-4-spinner-abierto.png)

### 🎯 Qué practicas
`ArrayAdapter` personalizado, `getView` y `getDropDownView`, `<string-array>`, `OnItemSelectedListener` ([07 §6](../02-conceptos/07-adaptadores.md)).
...
**🚀 Reto extra:** para un `Spinner` sencillo (sin fila personalizada) mira el atajo `ArrayAdapter.createFromResource(this, R.array.ej_cursos, android.R.layout.simple_spinner_dropdown_item)` ([07 §6.1](../02-conceptos/07-adaptadores.md)).
...
![grid](../adjuntos/img/n3-5-grid.png)
...
![recycler](../adjuntos/img/n3-6-recycler.png)

### 🎯 Qué practicas
`RecyclerView.Adapter`, `ViewHolder`, `LayoutManager` ([14](../02-conceptos/14-recyclerview.md)).
...
![picasso](../adjuntos/img/n3-7-picasso.png)
