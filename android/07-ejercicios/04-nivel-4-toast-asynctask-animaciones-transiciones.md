...
**Antes de empezar, lee:** [04 — Toast personalizado](../02-conceptos/04-toast-personalizado.md), [05 — AsyncTask e hilos](../02-conceptos/05-asynctask-e-hilos.md), [06 — Animaciones frame by frame](../02-conceptos/06-animaciones-frame-by-frame.md) y [08 — Transiciones](../02-conceptos/08-transiciones.md). Los proyectos de los que salen: [Diseobesos](../05-proyectos-alumno/Diseobesos.md), [EjercicioDiseno](../05-proyectos-alumno/EjercicioDiseno.md) y [AdapterDam2](../05-proyectos-alumno/AdapterDam2.md).
...
![toast](../adjuntos/img/n4-1-toast.png)

### 🎯 Qué practicas
`Dialog` sin fondo, `LayoutInflater.inflate(…, null)`, `Handler.postDelayed`, `final` y `MiActividad.this` en clases anónimas ([04](../02-conceptos/04-toast-personalizado.md)).
...
- **`Ej41ToastPerActivity.this`** y no `this`: dentro de una clase anónima `this` sería el listener, no la Activity ([04 §4.1](../02-conceptos/04-toast-personalizado.md)).
...
![async](../adjuntos/img/n4-2-async.png)

### 🎯 Qué practicas
`AsyncTask<Params, Progress, Result>`, `doInBackground`, `onProgressUpdate`, `onPostExecute`, `publishProgress`, `TypedArray`, getters ([05](../02-conceptos/05-asynctask-e-hilos.md), [12](../02-conceptos/12-hilos-en-profundidad.md)).
...
> ℹ️ **`AsyncTask` está obsoleto** desde Android 11 (Android Studio lo muestra tachado), pero **es lo que se usa en clase y compila y funciona**. Las alternativas modernas se explican en [05 §7](../02-conceptos/05-asynctask-e-hilos.md).
>
> 📌 **Variante del PDF** (`Runnable` + `ThreadExecutor` con `new Thread(command).start()`): allí `run()` toca las vistas **directamente desde el hilo secundario**. Es válido para entender qué es un hilo, pero **no es correcto** para tocar la interfaz (puede lanzar `CalledFromWrongThreadException`). Los proyectos reales (`ProgresoCaballo`, `ProgressAndando`) usan `AsyncTask`, que sí garantiza el salto al hilo principal. Está explicado en [05 §5](../02-conceptos/05-asynctask-e-hilos.md).
...
![frames](../adjuntos/img/n4-3-frames.png)

### 🎯 Qué practicas
`animation-list`, `AnimationDrawable`, `setBackgroundResource`, `start()` / `stop()` / `isRunning()` ([06](../02-conceptos/06-animaciones-frame-by-frame.md)).
...
![transiciones](../adjuntos/img/n4-4-transiciones-menu.png)

### 🎯 Qué practicas
`res/transition`, `TransitionInflater`, `setEnterTransition`, `ActivityOptionsCompat.makeSceneTransitionAnimation`, `overridePendingTransition` ([08](../02-conceptos/08-transiciones.md)).
...
### 🎯 Qué practicas
`Pair`, `ViewCompat.setTransitionName`, `VIEW_NAME_HEADER_IMAGE` como `public static final` ([08 §3](../02-conceptos/08-transiciones.md)).
