...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //TextWatcher = "vigilante" que se entera cada vez que cambia el texto de Re-password
        etRePassword.addTextChangedListener(new TextWatcher() {
            //Se ejecuta DESPUES de cada cambio de texto
            @Override
            public void afterTextChanged(Editable s) {
                String password = etPassword.getText().toString();
                String repassword = etRePassword.getText().toString();
                if (!password.equals(repassword)){
                    //No coinciden: desactiva el boton Nuevo y pone el campo en rojo
                    btnRegistrarNuevo.setEnabled(false);
                    etRePassword.setBackgroundColor(Color.RED);
                }else{
                    //Coinciden: activa el boton y quita el rojo
                    btnRegistrarNuevo.setEnabled(true);
                    etRePassword.setBackgroundColor(Color.WHITE);
                }
            }

            //Obligatorios por la interfaz TextWatcher, pero no los necesitamos (van vacios)
            @Override
