...
Ya analizado línea a línea en [02-diseno-basado-en-pesos](../conceptos/02-diseno-basado-en-pesos.md) §3: una cabecera (peso 0.5), un cuerpo con una rejilla 2×2 de botones (peso 4) y un pie (peso 0.5), todo construido con `LinearLayout` anidados y `layout_weight`, sin usar `ConstraintLayout` en absoluto.
...
```
Campos guardados como **atributos** de la Activity (ver [00-programacion-basica](../conceptos/00-programacion-basica.md) §5 si no sabes qué es un atributo) — aunque en este fichero en particular no se acaban usando desde fuera (a diferencia de `Caballo`/`Pikachu`, que sí los exponen a su `AsyncTask` correspondiente).
...
```
Un `Intent` explícito normal (ver [01-fundamentos-bundle-intent-ciclo-vida](../conceptos/01-fundamentos-bundle-intent-ciclo-vida.md) §3), sin extras ni animación.
...
> ```
> Es una nota recordatorio de que `findViewById` con **casteo** explícito (`(Button) findViewById(...)` — ver [00-programacion-basica](../conceptos/00-programacion-basica.md) §12 si no sabes qué es un cast) es la forma clásica y funciona igual de bien que la asignación con inferencia de tipo moderna (`Button btn = findViewById(...)`) — ambas son válidas, la segunda es simplemente más cómoda de escribir.
...
```
Esto es una variación interesante del patrón "toast con Dialog" (ver [04-toast-personalizado](../conceptos/04-toast-personalizado.md)): en vez de que el `Handler.postDelayed` simplemente cierre el diálogo, **primero lanza la Activity `Pikachu` y luego cierra el diálogo** — es decir, el "toast" hace de pantalla de carga/transición de 3 segundos antes de entrar de verdad a la animación.
...
```
Este es el "cierre manual" mencionado en [04-toast-personalizado](../conceptos/04-toast-personalizado.md) §4: en vez de un temporizador, el propio icono actúa de botón para cerrar el diálogo cuando el usuario lo desee.

## `Caballo.java` / `ProgresoCaballo.java` y `Pikachu.java` / `AnimacionPikachu.java`

Ambos pares de clases siguen **exactamente** el mismo patrón, ya explicado a fondo en [05-asynctask-e-hilos](../conceptos/05-asynctask-e-hilos.md) §3 (con `Caballo`/`ProgresoCaballo` como ejemplo principal) y en [06-animaciones-frame-by-frame](../conceptos/06-animaciones-frame-by-frame.md):
1. La Activity carga un `TypedArray` de imágenes (`R.array.imagenes` para el caballo, `R.array.pikachu` para Pikachu).
2. Expone sus vistas (`ImageView`, y en el caso del caballo también `ProgressBar`/`TextView`) mediante **getters** (métodos que solo devuelven el valor de un atributo — ver [00-programacion-basica](../conceptos/00-programacion-basica.md) §9).
...
Este proyecto usa a fondo las técnicas explicadas en [../conceptos/15-drawables-formas-y-selectores](../conceptos/15-drawables-formas-y-selectores.md) — merece la pena revisarlas ahí en detalle, aquí solo el resumen de qué archivo hace qué:
...
- El diseño de menú con pesos: es el único proyecto que usa `LinearLayout` + pesos en vez de `ConstraintLayout` para una pantalla completa — ver [02-diseno-basado-en-pesos](../conceptos/02-diseno-basado-en-pesos.md).
