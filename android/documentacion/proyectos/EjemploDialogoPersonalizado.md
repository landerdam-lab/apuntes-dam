...
> **Nota:** este documento sustituye a la versión anterior, que describía un esqueleto con solo un login "de mentira". El proyecto ahora es una **app completa de login + registro con base de datos local (Room)**. Sigue el PDF `pdfs/1-DialogSQLitePersonalizado.pdf`.
...
Capturas hechas ejecutando el proyecto en un emulador (Android 17). Para ejecutarlo tú, sigue la [Guía: cómo abrir, ejecutar y probar una app](../guias/01-ejecutar-la-app.md).

| 1. Inicio | 2. Diálogo de login | 3. Registro vacío |
|---|---|---|
| ![inicio](../img/01-pantalla-principal.png) | ![login](../img/02-dialogo-login.png) | ![registro](../img/03-registro-vacio.png) |

| 4. Contraseñas distintas | 5. Dos usuarios en la lista | 6. Clic largo: borrar |
|---|---|---|
| ![no coincide](../img/04-password-no-coincide.png) | ![lista](../img/07-lista-dos-usuarios.png) | ![borrar](../img/09-alertdialog-borrar.png) |

| 7. Clic corto: cargar usuario | 8. Aviso al cancelar | 9. Pantalla central |
|---|---|---|
| ![seleccionado](../img/08-usuario-seleccionado.png) | ![toast](../img/10-toast-no-se-elimino.png) | ![central](../img/11-pantalla-central.png) |
...
```
- **Acceder**: crea el diálogo y lo muestra con el gestor de fragmentos (ver [18-dialogos](../conceptos/18-dialogos.md)).
- **Registrar**: un `Intent` explícito a otra `Activity` (ver [11-intent-en-profundidad](../conceptos/11-intent-en-profundidad.md)).
- El bloque `EdgeToEdge` + `setOnApplyWindowInsetsListener` es el arranque estándar que se repite en todos los proyectos (ver [01-fundamentos](../conceptos/01-fundamentos-bundle-intent-ciclo-vida.md)).

## `model/Usuario.java` — la entidad

`@Entity(tableName = "Usuario")` convierte la clase en una **tabla** con tres columnas: `id` (clave primaria autogenerada), `usuario` y `password`. Tiene un constructor vacío (lo usa Room) y otro de dos parámetros marcado con `@Ignore` (lo usamos nosotros). Detalle completo en [17-room-base-de-datos](../conceptos/17-room-base-de-datos.md).
...
`@Database(entities = {Usuario.class}, version = 1, exportSchema = false)`, clase `abstract` con **Singleton** (`getInstance`, `LOCK`, `sInstance`) y el método `usuariosDao()` que da acceso al DAO. El fichero queda como `AppDatabase` (el PDF lo llamaba `UsuariosBD`). Ver [17](../conceptos/17-room-base-de-datos.md).

## `bbdd/AppExecutors.java` — hilos

Tres ejecutores: `getDiskIO()` (un hilo, para la BD), `getMainThread()` y `getNetworkIO()`. **Aviso:** el orden de los parámetros en `getInstance()` está cambiado respecto al constructor (igual que en el PDF), por lo que `getMainThread()` no devuelve realmente el hilo principal. Explicación y corrección en [20-executors-y-livedata](../conceptos/20-executors-y-livedata.md).
...
- **Mejora posible:** `getView` crea una fila nueva cada vez e ignora `convertView`. Reutilizarla (ver [07-adaptadores](../conceptos/07-adaptadores.md)) es más eficiente.
...
```
Se llama **una vez**. Después, cada insertar/actualizar/borrar dispara `onChanged` y la lista se redibuja sola (ver [20-executors-y-livedata](../conceptos/20-executors-y-livedata.md)).
...
Los listeners (`OnItemClick`, `OnItemLongClick`, `TextWatcher`) están explicados en [19-textwatcher-y-eventos-de-lista](../conceptos/19-textwatcher-y-eventos-de-lista.md) y el `AlertDialog` en [18-dialogos](../conceptos/18-dialogos.md).

## `fragmentos/LoginDialogFrag.java` — el login

Es un `DialogFragment` con el diseño `dialog_per.xml`. Sus métodos (`onCreateDialog`, `onCreateView`, `onViewCreated`) están descritos en [18-dialogos](../conceptos/18-dialogos.md). La lógica del botón **Aceptar**:
...
- **`dialog_per.xml`** — logo (`@drawable/logo_almi`, un `.jpg` en `res/drawable`), dos filas `TextView` + `EditText` con `layout_weight="1"` (ver [02-diseno-basado-en-pesos](../conceptos/02-diseno-basado-en-pesos.md)) y dos botones. `inputType="textPassword"` oculta la contraseña.
...
| Orden de parámetros en `AppExecutors` | `getMainThread()` no es el hilo principal (igual que en el PDF). Ver [20](../conceptos/20-executors-y-livedata.md). |
...
- **[Nivel 7 — Mejoras del proyecto real](../ejercicios/07-nivel-7-mejoras-del-proyecto.md)**: arreglar exactamente los fallos de la tabla de arriba (mensaje del diálogo, `AppExecutors`, login, actualizar sin seleccionar, validación, usuarios repetidos). Cada uno con su solución **probada en el emulador**. (El cifrado de contraseñas está como extra opcional, no visto en clase.)
- **[Nivel 6 — Diálogos y Room](../ejercicios/06-nivel-6-dialogos-y-room.md)**: los mismos pasos del PDF (`AlertDialog` → `DialogFragment` → entidad → DAO → BD → Activity) construidos desde cero en pequeño.
- **[Simulacro C — Login y registro con Room y diálogos](../ejercicios/08-simulacros-de-examen.md)**: un examen completo con este mismo proyecto.
- Código de todas las soluciones: `Desktop\android\proyectos-zip\EjemploDialogoPersonalizado-con-soluciones.zip`.

## Ver también
- [17 — Room](../conceptos/17-room-base-de-datos.md) · [18 — Diálogos](../conceptos/18-dialogos.md) · [19 — TextWatcher y eventos de lista](../conceptos/19-textwatcher-y-eventos-de-lista.md) · [20 — Executors y LiveData](../conceptos/20-executors-y-livedata.md)
- [16 — Fragmentos](../conceptos/16-fragmentos.md) — el `DialogFragment` es un tipo de fragmento.
- [07 — Adaptadores](../conceptos/07-adaptadores.md) — `ArrayAdapter`, `getView`, `notifyDataSetChanged`.
- [00 — Programación básica](../conceptos/00-programacion-basica.md) — `extends`, interfaces, clases anónimas, `static`, `null`.
