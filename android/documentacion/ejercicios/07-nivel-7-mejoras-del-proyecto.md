...
> ✅ **Verificado:** las mejoras 7.1 a 7.5 están aplicadas en `proyectos-zip\EjemploDialogoPersonalizado-con-soluciones.zip`, compiladas y **probadas en el emulador** (formulario vacío, avisos, duplicados, login incorrecto/correcto). Tu proyecto original en `AndroidStudioProjects` **no se ha modificado**.
...
### 🎯 Qué practicas
`AlertDialog.Builder.setMessage` ([18 §1](../conceptos/18-dialogos.md)).
...
|---|---|
| ![antes](../img/09-alertdialog-borrar.png) | ![despues](../img/12-alertdialog-con-mensaje.png) |
...
### 🎯 Qué practicas
Leer un constructor con **parámetros en un orden** y comprobar que se le pasan **en el mismo orden** ([20](../conceptos/20-executors-y-livedata.md)).
...
### 🎯 Qué practicas
`Toast`, `dismiss()`, código que corre tras volver al hilo principal con `AppExecutors` ([18 §2](../conceptos/18-dialogos.md), [20](../conceptos/20-executors-y-livedata.md)).
...
![aviso de login incorrecto](../img/13-login-incorrecto.png)
...
![actualizar sin seleccion](../img/15-actualizar-sin-seleccion.png)
...
**🎯 Qué practicas:** `TextWatcher`, un único vigilante en varios campos, un método de validación reutilizable ([19 §3](../conceptos/19-textwatcher-y-eventos-de-lista.md)).
...
![no coincide](../img/04-password-no-coincide.png)
...
### 🎯 Qué practicas
Una `@Query` nueva del DAO (igual que `loadUsuarioById`), consulta previa en segundo plano y volver al hilo principal con `AppExecutors` para el `Toast` ([17](../conceptos/17-room-base-de-datos.md), [20](../conceptos/20-executors-y-livedata.md)).
...
![aviso duplicado](../img/14-usuario-repetido.png)
